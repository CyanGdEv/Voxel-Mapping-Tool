...
3. Freeze the ranked document queue, assign every document exactly once across 20 planning shards, download into the private content-addressed cache, and record hashes/provenance. Each shard feeds all bounded raster pages through one CPU-sized worker pool, so a many-page drawing cannot leave the other shard cores idle.
...
5. A generation coordinator starts beside the extraction matrix, waits for exactly 20 identity-checked shard artifacts through the read-only Actions API, and integrity-merges them in frozen plan order as soon as the last artifact arrives. It then promotes only confidence-gated geometry from accepted applications with current-state/as-built evidence or independent DSM structural corroboration. The final build fails closed if the merged bundle is absent or invalid; it never silently repeats planning extraction. Manual manifests remain an expert override. Raw-document corpus consolidation runs after the generation workflow so cache maintenance cannot delay the world download.
...
- Generation does not wait for a second post-matrix runner. Its coordinator accepts only the current run/attempt's exact 20 shard artifacts, fails promptly on a failed shard, preserves deterministic shard/page order, and leaves corpus maintenance off the world-production critical path.
- Raw official planning documents are restored from one park-wide content-addressed corpus before sharding. Each shard copies only its assigned corpus hits into its active cache, then a follow-up cache workflow consolidates all shard deltas for later runs; a transient authority outage cannot discard evidence acquired by an earlier run.
