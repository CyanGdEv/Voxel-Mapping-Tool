...
Independent alignment, terrain and supplemental datasets are acquired in parallel. A frozen planning queue is distributed across 20 extraction jobs; every document and configured page is assigned exactly once at the same 300 DPI and evidence thresholds. Each shard owns its bounded page/archive retry, then a strict merge verifies all shard indexes, plan hashes, identities, coverage and frozen ordering before generation. The final build consumes that merged bundle with zero planning downloads or OCR/extraction retries. Drawing downloads and OCR derivatives remain in the private Actions cache when strict generation stops, so retries stay incremental without weakening planning-authority or current-state confidence gates.
