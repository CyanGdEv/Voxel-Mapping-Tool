...
Independent alignment, terrain and supplemental datasets are acquired in parallel. Official planning applications are crawled with bounded concurrency and a per-authority outage circuit breaker, while drawing downloads and OCR derivatives are retained in the private Actions cache even when strict generation stops. A failed run therefore uploads source diagnostics and makes the next run incremental without weakening planning-authority or current-state confidence gates.
